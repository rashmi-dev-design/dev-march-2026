# CI/CD Pipeline — java-maven-calculator-app

## Pipeline Overview

```
Checkout → Lint → Unit Test → Integration Test → Build → Docker Build → Docker Push → Deploy Staging → Deploy Production
```
...
| Unit Test | mvn test / Surefire | Fast unit tests |
| Integration Test | mvn verify / Failsafe | Full integration tests |
| Build | mvn package | Build WAR/JAR artifact |
...
| JUnit | Publish Surefire/Failsafe test results |
